...
export default function FbReloginButton() {
  ...
  if (phase.type === "idle") {
    return (
      <button
        onClick={startLogin}
        className="flex items-center gap-1.5 text-xs font-medium bg-accent hover:bg-accent/80 text-white rounded-lg px-2.5 py-1.5 transition-colors"
      >
        <LogIn size={11} />
  ...
  if (phase.type === "loading") {
    return (
      <span className="flex items-center gap-1.5 text-xs text-lo">
  ...
  if (phase.type === "needs_code") {
    return (
      <div className="flex flex-col gap-2 w-full">
        <p className="text-xs font-medium text-md flex items-center gap-1">
          <KeyRound size={11} className="text-lo" />
          FB sent you a verification code
        </p>
        <p className="text-xs text-lo">Check your email or SMS.</p>
        <div className="flex gap-1.5">
          <input
            ...
            maxLength={8}
            className="flex-1 text-xs border border-white/[0.08] rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-accent/25 focus:border-accent/50 min-w-0 bg-card text-hi placeholder-md"
          />
          <button
            onClick={submitCode}
            disabled={!code.trim()}
            className="text-xs font-medium bg-accent hover:bg-accent/80 disabled:opacity-40 text-white rounded-lg px-3 py-1.5 transition-colors shrink-0"
          >
            Submit
          </button>
        </div>
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-lo hover:text-md transition-colors self-start"
        >
          <Smartphone size={11} />
    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-1.5">
          <button
            onClick={checkApproval}
            className="flex items-center gap-1.5 text-xs font-medium bg-lift hover:bg-muted-bg text-md rounded-lg px-2.5 py-1.5 transition-colors"
          >
            <RefreshCw size={11} />
            Check again
          </button>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <button
        onClick={startLogin}
        className="self-start flex items-center gap-1.5 text-xs font-medium bg-accent hover:bg-accent/80 text-white rounded-lg px-2.5 py-1.5 transition-colors"
      >
        <LogIn size={11} />
        Try again
      </button>
